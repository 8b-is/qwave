...
| Apple's MusicKit services (subscription check, artwork, catalog metadata) | Now Playing HUD in the toolbar: shows what's playing in Apple Music | Only if you have an active Apple Music subscription **and** something is playing; authorization is requested the first time the HUD would appear, never on launch | **Yes** — no subscription or nothing playing means the HUD never appears and no request is made (`MusicHUDPresenceMachine`, see [docs/MUSIC.md](MUSIC.md)). This traffic goes through Apple's system `MusicKit`/`MediaPlayer` frameworks, not Qwave's own `URLSession` — it is not interceptable or auditable by Qwave's egress allowlist, the same way `Category C`'s Safe Browsing check is Apple's own path rather than Qwave's. | `AppleMusicKit/AppleMusicSession.swift`; disclosure `docs/MUSIC.md` |
...
- **Stored memory bodies never go — by a declared-intent gate plus a
  ...
  `provider.kind == .onDevice` (`WaveDirector.swift:338-344`). The context handed
  to the policy is built by the same caller (`WaveDirector.swift:329`; `ask`
  sets `includeStoredMemory` only for the on-device provider at
  `WaveDirector.swift:303`). Both halves hold with a remote provider configured
  and a key entered.
