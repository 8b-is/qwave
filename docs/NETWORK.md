...
comment. It is gated now. The user-configurable half is carried by
`EgressAllowlist.userConfiguredHost` — **one** slot, **exact match** (unlike
the committed list, which matches subdomains: typing `example.com` is not
consent for `telemetry.example.com`), written from exactly one place,
`WaveDirector.resolveProvider()`, out of the same preference the base URL
comes from. Pointing the provider at a new endpoint revokes the previous host
in the same call; switching to the on-device or no provider clears it. So the
guard permits the committed hosts plus, at most, the one host you configured.
