...
- **The page text goes.** "Summarize Page" sends the extracted article body,
  ...
  (`WaveDirector.swift:265-268` → `ArticleExtract.clamped()` at
  `MemoryRecord.swift:64`, `MemoryWaveConstants.maxPageCharacters`). "Ask"
  appends the same clamped page text to your question
  (`WaveDirector.swift:300-301`). If you would not paste a page into that
...
- **Stored memory bodies never go — by a declared-intent gate plus a
  ...
  prompt is `WaveDirector`'s own guard — the recall block is appended only when
  `provider.kind == .onDevice` (`WaveDirector.swift:370-377`). The context handed
  to the policy is built by the same caller (`WaveDirector.swift:355-364`; `ask`
  sets `includeStoredMemory` only for the on-device provider at
  `WaveDirector.swift:319`). Both halves hold with a remote provider configured
  and a key entered. Say plainly which half is load-bearing: the caller-side
  guard is. The policy is a tripwire behind it, dormant in normal operation
  precisely because `ask` never declares the dangerous pair — it is what fails
  if a future caller does. `infer` hands it the intent as declared, without
  pre-filtering, or it could not be tripped at all.
- **Timeline summaries are the titles-and-times case.** Summarising your
  ...
  `WaveDirector.swift:153-154`; `MemoryTimeline.swift:63-79`).
...
startup** — and that is the one claim on this page checked dynamically rather
than by review: `EgressGuardTests` registers a `URLProtocol` recorder, drives
the shields launch path, and asserts zero requests were observed
...
the app does at startup. It remains the only dynamic assertion here — it is just
not a process-wide egress monitor.
