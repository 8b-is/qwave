...
- **Stored memories never go — and that is enforced, not merely intended.**
  `MemoryWavePolicy.decide` returns `.deny(.cognitiveEgress)` for *any* remote
  request that would carry stored memories, so the request is refused before it
  is built (`MemoryWavePolicy.swift:81-82`). `WaveDirector` only ever sets
  `includeStoredMemory` for the **on-device** provider
  (`WaveDirector.swift:303`), and the policy is the fail-closed backstop behind
  that choice. This holds even with a remote provider configured and a key
  entered.
...
startup** — and *that* one is proved dynamically: `EgressGuardTests`
registers a `URLProtocol` recorder, runs the real launch shields path, and
asserts zero requests were observed
(`EgressGuardTests.swift:97-130`). That test has teeth.
