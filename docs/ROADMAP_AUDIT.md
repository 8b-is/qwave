...
| Item | Status | Evidence | Gap / notes |
|---|---|---|---|
| Negative/tamper KAT — ML-KEM-768 (bit-flip changes shared secret) | ✅ shipped | `Packages/QwaveKit/Tests/PostQuantumTests/MLKEM768Tests.swift:74 testImplicitRejectionOnTamperedCiphertext`<br>`Packages/QwaveKit/Tests/PostQuantumTests/HybridKEMNegativeSuite.swift:77 mlkemBitFlipChangesSharedSecret`<br>`Packages/QwaveKit/Sources/PostQuantum/MLKEM768.swift:428 constant-time implicit rejection` |  |
| Negative/tamper KAT — Classic McEliece 348864 (bit-flip throws) | ✅ shipped | `Packages/QwaveKit/Tests/PostQuantumTests/ClassicMcEliece348864Tests.swift:85 testTamperedCiphertextThrows`<br>`Packages/QwaveKit/Tests/PostQuantumTests/HybridKEMNegativeSuite.swift:92 mcelieceBitFlipThrows`<br>`Packages/QwaveKit/Sources/PostQuantum/ClassicMcEliece348864.swift:548 McElieceError.decodingFailure` |  |
| Negative/tamper KAT — HybridKEM | ✅ shipped | `Packages/QwaveKit/Tests/PostQuantumTests/HybridKEMNegativeSuite.swift:35 keygenRejectsWrongSeedSize`<br>`Packages/QwaveKit/Tests/PostQuantumTests/HybridKEMNegativeSuite.swift:41 encapsulateRejectsWrongEKSize`<br>`Packages/QwaveKit/Tests/PostQuantumTests/HybridKEMNegativeSuite.swift:48 decapsulateRejectsTruncatedKey`<br>`Packages/QwaveKit/Tests/PostQuantumTests/HybridKEMNegativeSuite.swift:60 decapsulateRejectsTruncatedCiphertext` |  |
...
| docs/CRYPTO_REVIEW.md present and matching code | 🟡 partial | `docs/CRYPTO_REVIEW.md:1 present`<br>`docs/CRYPTO_REVIEW.md:21 F1 constant-time re-encryption check -> Packages/QwaveKit/Sources/PostQuantum/MLKEM768.swift:428-440 (byte-wise OR-of-XOR acc + branchless mask select)`<br>`docs/CRYPTO_REVIEW.md:77 F6 boundary throws -> HybridKEM.swift:19 invalidInputSize`<br>`docs/CRYPTO_REVIEW.md:86 F7 fail-closed -> QuantumSessionPolicy.swift:26 + PacketTunnelProvider.swift:124` | Document is present and every technical claim (F1 constant-time compare, F6 throwing boundary + HybridKEMNegativeSuite, F7 negotiateFailClosed/downgradeBlocked/FailClosedNegotiationTests, daily-rekey-keeps-PSK) matches current sources. Staleness caveat: header (docs/CRYPTO_REVIEW.md:3) states it was reviewed 2026-08-13 against v0.3.0 sources, but the repo is released at v0.5.0 and the doc was not re-stamped; no evidence of a re-review at v0.5.0. Separately, docs/VPN_STAGE_B.md:19-21 (the Mullvad reference section) cites ML-KEM-1024 + McEliece 460896f while Qwave actually ships ML-KEM-768 + McEliece 348864 (correctly stated in VPN_STAGE_B.md:73-87). |

### UX debt (daily-driver features) in the AppKit shell / BrowserCore

| Item | Status | Evidence | Gap / notes |
|---|---|---|---|
| Omnibox suggestions (address-bar autocomplete/history/search) | 🟡 partial | `Packages/QwaveKit/Sources/BrowserCore/OmniboxSuggester.swift:26-87 (ranked history matcher: host-prefix > url-prefix > substring, frequency+recency scoring, dedupe, bounded top-N)`<br>`Sources/QwaveApp/OmniboxSuggestionsController.swift:9-170 (non-activating dropdown panel, NSTableView, keyboard highlight, click-commit)`<br>`Sources/QwaveApp/BrowserWindowController.swift:1036-1041 (history.entries(matching:limit:50) -> OmniboxSuggester.suggestions -> show)`<br>`Sources/QwaveApp/BrowserWindowController.swift:1054-1069 (arrow/return/escape forwarded from field editor to moveHighlight/hide)` | History-based autocomplete + suggestions dropdown are fully implemented, wired, and unit-tested. What is missing: remote search-engine suggestions. OmniboxSuggester.suggestions only takes a local `history: [HistoryEntry]` array (OmniboxSuggester.swift:27-32); there is no network call or search-provider suggestion API anywhere in the path. Rows show no favicon (OmniboxSuggestionsController.swift:140-167 renders text only). |
| Favicons (fetch + cache + display) | 🟡 partial | `Sources/QwaveApp/FaviconLoader.swift:11-68 (ephemeral cookieless URLSession fetch, 512KB cap, HTTP-200 guard, resize to 16x16)`<br>`Packages/QwaveKit/Sources/BrowserCore/NavigationCoordinator.swift:246-260 (extractFaviconURL: JS reads link[rel~=icon]/shortcut icon, falls back to /favicon.ico, http/https only)`<br>`Sources/QwaveApp/BrowserWindowController.swift:261-269 (coordinator.onFaviconURL -> faviconLoader.load -> tab.favicon = image)`<br>`Sources/QwaveApp/BrowserWindowController.swift:310-313, 369-372 (tab.favicon ?? faviconLoader.cachedIcon fed into TabDisplayModel)` | Fetch, per-container-keyed cache, and display in the tab strip are all implemented. Two gaps: (1) cache is in-memory only (FaviconLoader.swift:12 `cache: [String: NSImage]` dictionary, cleared on relaunch) with no on-disk persistence; (2) favicons appear in tabs but not in the omnibox suggestions dropdown or (unverified) history/Library rows — the suggestion cell renders text only. No test file covers FaviconLoader. |
...
| Item | Status | Evidence | Gap / notes |
|---|---|---|---|
| MV3 content_scripts injection into pages | 🟡 partial | `Packages/QwaveKit/Sources/WebExtensions/MV3Manifest.swift:12`<br>`Packages/QwaveKit/Sources/WebExtensions/MV3Manifest.swift:14`<br>`Packages/QwaveKit/Sources/WebExtensions/MV3Manifest.swift:25`<br>`Packages/QwaveKit/Sources/WebExtensions/MV3Manifest.swift:42` | Only the manifest MODEL is shipped: content_scripts (matches/js/run_at) decode into MV3Manifest.ContentScript and a test asserts the parse (WebExtensionsTests.swift:17-28). There is NO injection engine. installBridge (WebExtensionHost.swift:25-34) injects ONLY the browser.* bridge (BrowserBridgeScript) forMainFrameOnly at .atDocumentStart; nothing reads manifest.contentScripts, matches URLs against `matches` patterns, loads the declared `js` files, or injects them into visited pages. The ContentScript.runAt field is decoded but never mapped to a WKUserScript injectionTime — the only injectionTime usages (WebExtensionHost.swift:30, ExtensionPopupController.swift:23) are hardcoded for the bridge. Grep for `contentScript` outside MV3Manifest.swift returns no injection call sites. Missing: URL match-pattern matching, per-extension content-script user-script installation, run_at mapping. |
| declarativeNetRequest subset (static/dynamic rules) | 🔴 missing | `grep declarativeNetRequest over Packages/QwaveKit/Sources/WebExtensions and Tests/WebExtensionsTests: 0 hits`<br>`Packages/QwaveKit/Sources/WebExtensions/MV3Manifest.swift:11` | No declarativeNetRequest support of any kind. No rule model, no static rules file (rule_resources) parsing, no dynamic rule API, no WKContentRuleList / URLSchemeHandler wiring. manifest.permissions is decoded as a flat [String] (MV3Manifest.swift:11) and never acted upon; there is no declarative_net_request manifest key in CodingKeys (lines 20-26). Zero source and zero test references. |
| browser.tabs bridge (query/create) | 🟡 partial | `Packages/QwaveKit/Sources/WebExtensions/ExtensionMessageRouter.swift:91`<br>`Packages/QwaveKit/Sources/WebExtensions/ExtensionMessageRouter.swift:95`<br>`Packages/QwaveKit/Sources/WebExtensions/ExtensionMessageRouter.swift:35`<br>`Packages/QwaveKit/Sources/WebExtensions/BrowserBridgeScript.swift:54` | Real routing but delegated to host-supplied callbacks, not implemented in QwaveKit. Only tabs.query and tabs.create are wired (router.swift:91-98); both forward to optional closures tabQueryHandler/tabCreateHandler (declared lines 35-36). If the app leaves them unset, tabs.query returns [] and tabs.create is a silent no-op (`?? []` line 93, `?.` line 97). No tabs.update/remove/get/onUpdated etc. Tests set the closures manually, so coverage proves the plumbing, not a concrete tab implementation. |
...
| browser.runtime.sendMessage / onMessage bridge | 🟡 partial | `Packages/QwaveKit/Sources/WebExtensions/ExtensionMessageRouter.swift:99`<br>`Packages/QwaveKit/Sources/WebExtensions/ExtensionMessageRouter.swift:39`<br>`Packages/QwaveKit/Sources/WebExtensions/ExtensionMessageRouter.swift:101`<br>`Packages/QwaveKit/Sources/WebExtensions/BrowserBridgeScript.swift:49` | sendMessage routing is real and supports sync + async single reply via runtimeMessageHandler (router.swift:99-107; tested with immediate and 10ms-delayed replies). But like tabs it is a host hook: if runtimeMessageHandler is unset the call just resolves to null (router.swift:101-104) — QwaveKit ships no default message dispatch. The receiving half is a stub: BrowserBridgeScript.swift:52 defines runtime.onMessage.addListener/removeListener as empty no-ops, so an extension background/content script cannot actually receive messages through the bridge. Missing: onMessage delivery, sender metadata, multi-listener fan-out. |
